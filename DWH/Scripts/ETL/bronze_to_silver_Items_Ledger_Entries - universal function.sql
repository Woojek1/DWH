...
DO $$
DECLARE
    -- Tablica z nazwami firm wykorzystywana w pętli
    _firmy         text[] := ARRAY['aircon','zymetric','technab'];
    _firma         text;
    _tabela        text;
    _litera_firmy  char(1);
...
BEGIN
    FOREACH _firma IN ARRAY _firmy LOOP
        _litera_firmy := UPPER(SUBSTR(_firma,1,1));
        _tabela := format('bc_items_ledger_entries_%s', _firma);

        -- 1) Tworzenie tabeli głównej ze wskazaniem partycjonowania
        EXECUTE format($ddl$
            CREATE TABLE IF NOT EXISTS silver.%I (
                "Entry_No"                   int4          NOT NULL,
        ...
        -- 2) Automatyczne tworzenie partycji rocznych (ostatnie 5 lat, bieżący i przyszły)
        FOR yr IN EXTRACT(YEAR FROM CURRENT_DATE)::int - 5 .. EXTRACT(YEAR FROM CURRENT_DATE)::int + 1 LOOP
            EXECUTE format($part$
                CREATE TABLE IF NOT EXISTS silver.%I_%s
                    PARTITION OF silver.%I
            ...
            $part$,
                ...
                yr,
                _tabela,
                make_date(yr, 1, 1)::text,
                make_date(yr+1, 1, 1)::text
            );
        END LOOP;

        -- 3) Pierwsze ładowanie danych z bronze do silver
        EXECUTE format($insert$
            INSERT INTO silver.%I (
                "Entry_No"
                ,"Posting_Date"
...
CREATE OR REPLACE FUNCTION bronze.fn_upsert_bc_items_ledger_entries()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
DECLARE
    firma          TEXT := TG_ARGV[0];
    litera_firmy   CHAR(1) := UPPER(SUBSTR(TG_ARGV[0],1,1));
    target_table   TEXT := format('bc_items_ledger_entries_%s', TG_ARGV[0]);
BEGIN
    EXECUTE format($etl$
        INSERT INTO silver.%I (
            "Entry_No"
...
-------------------------------------------------------------------------  
-- CREATING TRIGGERS IN BRONZE LAYER FOR INSERT/UPDATE ON SOURCE TABLES  
-------------------------------------------------------------------------

DO $$
DECLARE
    grupa_tabel text := 'items_ledger_entries';
    firmy       text[] := ARRAY['aircon','zymetric','technab'];
    firma       text;
BEGIN
    FOREACH firma IN ARRAY firmy LOOP
        EXECUTE format($sql$
            ...
            CREATE TRIGGER trg_upsert_bc_%I_%I
              AFTER INSERT OR UPDATE ON bronze.bc_%I_%I
              FOR EACH ROW
            EXECUTE FUNCTION bronze.fn_upsert_bc_items_ledger_entries(%L);
        $sql$,
            grupa_tabel, firma,
            grupa_tabel, firma,
            grupa_tabel, firma,
