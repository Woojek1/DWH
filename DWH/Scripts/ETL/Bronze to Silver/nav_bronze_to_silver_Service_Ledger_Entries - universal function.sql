...
BEGIN
    FOREACH _firma IN ARRAY _firmy LOOP
        ...
        _litera_firmy := UPPER(SUBSTR(_firma, 1, 1));
        _tabela_silver := format('nav_service_ledger_entry_%s', _firma);
        _tabela_bronze := format('nav_service_ledger_entry_%s', _firma);
        _tabela_items := format('bc_items_%s', _firma);
        ...
        $ddl$, _tabela_silver, _litera_firmy);
        ...
        EXECUTE format($insert$
            ...
            SELECT
                ...
                sle."Service Item No_ (Serviced)",
                sle."Item No_ (Serviced)",
                it."ednTypeA",  -- NIE MA W NAV
                it."ednTypeB",  -- NIE MA W NAV
                sle."Serial No_ (Serviced)",
                sle."Contract Invoice Period",
                sle."Global Dimension 1 Code", 	
                sle."Global Dimension 2 Code",
                ...
                sle."Type",
                sle."No_",
                it2."ednTypeA",  -- NIE MA W NAV
                it2."ednTypeB",  -- NIE MA W NAV
                sle."Cost Amount",
                sle."Discount Amount",
                sle."Unit Cost",
                sle."Quantity",
                ...
                CURRENT_TIMESTAMP
            FROM bronze.%I sle
            LEFT JOIN bronze.%I it
                ON sle."Item No_ (Serviced)" = it."No"
			LEFT JOIN bronze.%I it2
				ON sle."No_" = it2."No"
        $insert$, 
        _tabela_silver, 
        _litera_firmy, 
