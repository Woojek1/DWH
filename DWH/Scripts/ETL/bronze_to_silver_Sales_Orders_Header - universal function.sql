...
BEGIN
	...
	_tabela := format('bc_sales_orders_header_%s', _firma);  --- ZMIENIĆ NAZWĘ TABELI ŹRÓDŁOWEJ I DOCELOWEJ ---
...
CREATE OR REPLACE FUNCTION bronze.fn_upsert_bc_sales_orders_header()  -- ZMIENIĆ NAZWĘ FUNKCJI
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
DECLARE
...
-- litera := TG_ARGV[1];
	target_table := format('bc_sales_orders_header_%s', firma);  -- ZMIENIĆ NAZWĘ TABELI DOCELOWEJ --
...
DO $$
DECLARE
	grupa_tabel text := 'sales_orders_header'; 		-- ZMIENIĆ NAZWĘ GRUPY TABEL
