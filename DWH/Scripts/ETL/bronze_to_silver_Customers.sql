...
-- DROP TABLE silver.bc_customers_zymetric;

CREATE TABLE IF NOT EXISTS silver.bc_customers_zymetric (
	"No" text PRIMARY KEY
...
--------------------------------------
-- FIRST LOAD DATA TO SILVER CUSTOMERS
--------------------------------------


INSERT INTO silver.bc_customers_zymetric (
	"No"
	,"Contact_Type"
	,"Name"
	,"Name_2"
...
FROM bronze.bc_customers_zymetric c
...
CREATE OR REPLACE FUNCTION bronze.fn_upsert_bc_customers_zymetric()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
	BEGIN		INSERT INTO silver.bc_customers_zymetric (
...
CREATE TRIGGER trg_after_upsert_bc_customers_zymetric
	AFTER INSERT OR UPDATE
	ON bronze.bc_customers_zymetric
	FOR EACH ROW
	EXECUTE FUNCTION bronze.fn_upsert_bc_customers_zymetric();
